...
import { Settings, Save, CheckCircle2, AlertTriangle, Landmark, Smartphone, ShieldAlert, Building2 } from "lucide-react";
...
export default function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"GENERAL" | "FINANCIAL" | "PAYMENT" | "POLICIES">("GENERAL");
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage("System configuration saved successfully and audit entry created.");
      loadSettings();
    } catch (err: unknown) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Operational policies, financial rates, payment settlement accounts, and print templates.
          </p>
        </div>
        ...
        <Card>
          <CardHeader className="pb-3 border-b border-slate-100">
            <CardTitle className="text-sm font-semibold">
              ...
              {activeTab === "PAYMENT" && "Telebirr & Commercial Bank of Ethiopia (CBE) Settings"}
              {activeTab === "POLICIES" && "Automated Rules & Receipt Print Customization"}
            </CardTitle>
            <CardDescription className="text-xs">
              Changes take effect immediately across all front desk sessions and receipts
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="p-5 space-y-4 text-xs">
            {loading ? (
              ...
              tabSettings.map((s) => (
                <div key={s.key} className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center border-b border-slate-100 pb-3 last:border-0 last:pb-0">
                  <div>
                  ...
                  <div className="md:col-span-2">
                    {s.key === "calendar_display_mode" ? (
                      ...
                      >
                        <option value="DUAL">DUAL — Show Both Gregorian (GC) & Ethiopian Calendar (EC)</option>
                        <option value="GC_ONLY">GC_ONLY — Show Gregorian Calendar Only</option>
                      </select>
                    ) : (
                      <Input
          ...
          <CardFooter className="border-t border-slate-100 bg-slate-50/50 p-4 flex justify-between items-center">
            <div className="flex items-center space-x-1.5 text-slate-500 text-[11px]">
              ...
              <span>All configuration updates are permanently timestamped in the audit log.</span>
            </div>
